...
> **仓库形态**：`shijieweb/agent-meeting` 是 **monorepo** —— `skill/`（本技能）与 `server/`（Agent Hub 后端服务 + 网页前端）并列维护。安装脚本**仅部署 `skill/`** 到 WorkBuddy 技能目录，后端需另行按顶层 README 启动。
...
本仓库是 **monorepo**：`skill/`（本技能）与 `server/`（Agent Hub 后端 + 前端）并列。安装脚本只部署 `skill/` 子目录到 WorkBuddy 技能目录。手动安装：

```bash
git clone https://github.com/shijieweb/agent-meeting.git /tmp/agent-meeting
mkdir -p ~/.workbuddy/skills/agent-meeting
cp -R /tmp/agent-meeting/skill/. ~/.workbuddy/skills/agent-meeting/
```

> 一键安装（`curl ... | bash` / `irm ... | iex`）会自动完成上述「只取 skill/」的动作，推荐用一键方式。
