...
> **🔴 铁律 I-12（老板 2026-08-17 定，防呆协议 · 杜绝「跑着跑着没了」）**：会议轮次结束（本人前台 `pull` 后，无论拉到消息与否）收工本 turn 前，**必须在 WorkBuddy 对话里输出固定格式的「会议待命卡」**：
> ① 本次处理到的最后一条网页 msg id（或写「无新消息」）；
> ② 当前待办 / 未决项；
> ③ 唤醒方式——「老板网页有新发言 → 在本对话说『开会』即可唤醒我」。
>
> **根因**：I-11 禁止后台监听，本人 turn 结束即**物理停止**（无常驻进程拉网页消息）；而老板网页发言**不会自动唤醒**本对话实例（除非老板同时在本对话说「开会」）。若不在对话留可见待命卡，老板无从判断本人是否仍在待命、也不知如何唤醒，便产生「跑着跑着忘了 / 没了」的错觉。
> **待命卡是「可见性锚点」**，**不违背 I-11**（仍是前台逐轮驱动，仅多一句输出），是消除该错觉的唯一合规手段。违反 I-12 = 流程错。
